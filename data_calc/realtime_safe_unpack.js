...
setInterval(()=>{
    ...
    isGetPending = true;
    let fileDir = `/home/share/safe_calc/log/realtime_data/${svrArray[svrIdx]}/${year}/${month}${day}`;
    let fileNameNoExt = `/home/share/safe_calc/log/realtime_data/${svrArray[svrIdx]}/${year}/${month}${day}/${year}-${month}-${day}-${hourArray[hourIdx]}`;
    unpackLogFile(fileDir,fileNameNoExt,svrArray[svrIdx],year,month,day,hourArray[hourIdx]);
    index++;
...
脚本名称：unpackproc.sh
*/
function unpackLogFile(fileDir,fileNameNoExt,svrName,year,month,day,hour){
    let free = spawn('/home/share/safe_calc/log/realtime_safe_unpackproc.sh', [`${fileDir}`,`${fileNameNoExt}`,`${svrName}`,`${year}`,`${month}`,`${day}`,`${hour}`]);
