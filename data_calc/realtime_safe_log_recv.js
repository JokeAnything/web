...
let logRoot = "/click_txt/safe_realtime/data/";
...
function getSpecificHourLog(svr,hour){
    ...
    let chkTargetFile = "";
    // 优先创建日志目录
    let logDir = `realtime_data/${svr}/${year}/${month}${day}`;
    mkdirsSync(logDir);

    console.log(`${svr}-${hour}`);

    let ftpLogFile = `/click_txt/safe_realtime/data/${svr}/${year}/${month}${day}/${year}-${month}-${day}-${hour}.gz`;
    ...
    try{
        ftpObj.get(ftpLogFile,function(err,stream){
            ...
            logTargetFile = `realtime_data/${svr}/${year}/${month}${day}/${year}-${month}-${day}-${hour}.gz`;
...
/*    
    let ftpLogChk = `/click_txt/safe_realtime/data/${svr}/${year}/${month}${day}/${year}-${month}-${day}-${hour}.md5`;
    try{
